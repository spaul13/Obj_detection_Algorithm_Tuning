...
def cal_temporal_pairwise(i):
    ...
    if(True):
        ...
        os.system("grep 'Slice:I' log_1209.txt > log_temp.txt")
        I_frame_size_only = find_size()
        os.system(encode_temporal_pair)
        os.system("grep 'Slice:P' log_1209.txt > log_temp.txt")
        P_frame_size = find_size()
        os.system("grep 'Slice:I' log_1209.txt > log_temp.txt")
        I_frame_size = find_size()
        stat_info = os.stat("pic_0.png")
        filesize_1 = stat_info.st_size
        stat_info = os.stat("pic_1.png")
