#using PIL library, no preloading required (can operate on higher images)
#loading using PIL is much faster, resizing takes most of the time (as opposed to cv2)
...
traindir = "new_training_reduced_set\\"
...
batch_size = 4#3#1#4 #batch_size for training
log_step = 620 #990 #450
comp_techs = 3
path_saved_model = "C:\\temp\\sibu_0330_opencv_softmax_model"
...
#                    #print("[epoch:%d, batch:%5d] loss: %.3f" % (epoch + 1, i + 1, running_loss / float(2000)))
    ...
    def myloss(self, outputs, labels):
        ...
        for i in range(len(file_index_list)):
            ...
            """
            for i in range(10):
                if (reso_weight >= 0.05 and reso_weight < 1):
                    break
                elif (reso_weight > 1):
                    reso_weight = 1
                    break
                    ...
                    reso_weight *= 10
            for i in range(10):
                if (jpeg_weight >= 0.1 and jpeg_weight < 1):
                    break
                elif (jpeg_weight > 1):
                    jpeg_weight = 1
                    break
                    ...
                    jpeg_weight *= 10
            for i in range(10):
                if (encode_weight >= 0.1 and encode_weight < 1):
                    break
                elif (encode_weight > 1):
                    encode_weight = 1
                    break
                else:
                    encode_weight *= 10
    ...
    def compress(self, infile, file_index, reso_weight, jpeg_weight, encode_weight):
        ...
        if(width < 100):
            width = 100
            height = 100
        jpeg_qp = int(best_jpeg*jpeg_weight)#higher the better (base+enhancement)
        #if(jpeg_qp<10):
        #    jpeg_qp = 10
        temp_jpeg = "temp_4.jpg"
        tempfile = "temp_4.mp4"
        outfile = "temp_4_org.png"
        #loading the file, downsizing and JPEG compress (PIL)
        """
        foo = Image.open(infile)
        foo = foo.resize((width,height),Image.ANTIALIAS)
        jpeg_qp = int(best_jpeg*jpeg_weight)#higher the better
        foo.save(temp_jpeg,quality=jpeg_qp)
        """
        #loading the file, downsizing and JPEG compress (opencv)
        img = cv2.imread(infile, cv2.IMREAD_UNCHANGED)
        resized = cv2.resize(img, (width,height), interpolation = cv2.INTER_AREA)
        cv2.imwrite(temp_jpeg, resized, [int(cv2.IMWRITE_JPEG_QUALITY), jpeg_qp])
        #encoding qp also needs to be within a range
        #encoding_qp = int(best_qp/encode_weight)
        encoding_qp = int(best_qp/encode_weight)
        cmd_str = "ffmpeg -threads 4 -i " + temp_jpeg +" -c:v libx264 -qp " + str(encoding_qp) +" -loglevel quiet -y "+ tempfile
        os.system(cmd_str)
        ...
        """
        cmd_str = "ffmpeg -threads 4 -i " + tempfile +" -vf scale=3840:2160 -loglevel quiet -y " + outfile
        os.system(cmd_str)
        """
    ...
    def find_acc_loss(self, orgfile, testfile):
        ...
        return 2.0*(1.0 - acc_loss) #1.0 --> accuracy for groundtruth
