...
#        
#        scale_indices = li
    # creating the file to write the detection
...
#        print(end - start)
        ...
        for im_num, image in enumerate(imlist[i*batch_size: min((i +  1)*batch_size, len(imlist))]):
            ...
            print("# of objects detected = %d and prediction size = %d" %(len(objs), (prediction.size())[0] ))
            if(len(objs) > 0):
                piclist.append(image)
                picnumber.append(int((image.split("\\")[-1]).split("_")[1]))
            else:
        ...
        if(predict_dominant_class):
            print(prediction)
            temp = (prediction.size())[0] 
            count_sp, dominant_cls_index, dominant_cls_value = 0, 0, 0.0
            while(count_sp < temp):
                if(dominant_cls_value < prediction[count_sp,5].item()):
                    dominant_cls_value = prediction[count_sp,5].item()
                    dominant_cls_index = int(prediction[count_sp,7].item())
                count_sp+=1
            dominant_cls_list.append(dominant_cls_index)
            print("\n dominant class = %d"%dominant_cls_index)
