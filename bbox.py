...
def sanity_fix(box):
    ...
    return box
# here the input should be torch.tensor(numpy_array)
...
def bbox_iou_sp(box1, box2):
    ...
    """
    #Get the coordinates of bounding boxes
    b1_x1, b1_y1, b1_x2, b1_y2 = torch.tensor(box1[:,0]).cuda(), torch.tensor(box1[:,1]).cuda(), torch.tensor(box1[:,2]).cuda(), torch.tensor(box1[:,3]).cuda()
    b2_x1, b2_y1, b2_x2, b2_y2 = torch.tensor(box2[:,0]).cuda(), torch.tensor(box2[:,1]).cuda(), torch.tensor(box2[:,2]).cuda(), torch.tensor(box2[:,3]).cuda()
    print(b1_x1.shape.item())
    #get the corrdinates of the intersection rectangle
