...
#                    print("[epoch:%d, batch:%5d] loss: %.3f" % (epoch + 1, i + 1, running_loss / float(2000)))
    ...
    def compress(self, infile, file_index, reso_weight, jpeg_weight, encode_weight):
        ...
        os.system(cmd_str)
        """
        cmd_str = "ffmpeg -i " + infile +" -vf scale=" + str(width)+":" + str(height) + " -q:v "+ str(jpeg_qp) +" -c:v libx264 -qp " + str(encoding_qp) + " temp.mp4"
        os.system(cmd_str)
        ...
        """
        cmd_str = "ffmpeg -i temp.mp4 -vf scale=3840:2160 temp_1_org.png"
        os.system(cmd_str)
