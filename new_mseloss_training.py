...
traindir = "H:\\from_pytorch\\reduced_regression_training_fol\\"
...
batch_size = 8#16
log_step = 75
path_saved_model = "C:\\temp\\sibu_0406_mse_model"
save_model_step = 500
org_reso = 3840
total_configs = 107
    ...
    class Net(nn.Module):
        def __init__(self):
            ...
            self.fc_seqn = nn.Sequential(
                ...
                nn.ReLU(inplace=True),
                nn.Linear(256, 107)#total_configs)
            )
        def forward(self, x):
            x = self.conv_seqn(x)
            # flatten
    ...
    def training_code(self, model):        
        ...
        for epoch in range(self.epochs):  
            ...
            for i, data in enumerate(self.train_data_loader):
                ...
                output_actual = model(inputs)
                #print(outputs)
                #print(outputs.size())
                values, outputs = torch.max(output_actual, 1)
                print(values, outputs)
                print(labels)
                #print("\n the outputs are : ", outputs)
                #to get only postive outputs need to apply softmax
                outputs = torch.nn.functional.softmax(output_actual)
                print(outputs)
                #outputs = torch.clamp(outputs, min=0.1, max=1)
                #labels = torch.unsqueeze(labels, 1)#need to uncomment for model with output node 1
                loss = criterion(output_actual.float(), labels)
                loss.backward()
                optimizer.step()
...
def main():
    ...
    """
    model_saved_path = "C:\\temp\\mse_weight_pytorch\\sibu_0406_mse_model_1500"
