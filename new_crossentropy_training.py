...
#traindir = "H:\\regression_training_fol\\"
traindir = "H:\\small_regression_fol\\"
...
momentum = 0.9
rect_size = 224
image_size = [3840,3840]#[rect_size, rect_size]
batch_size = 1
log_step = 150 
path_saved_model = "C:\\temp\\sibu_0408_crossentropy_model"
...
class Detection_net(nn.Module):
    ...
    def training_code(self, model):        
        ...
        criterion = nn.CrossEntropyLoss().to(device)#Combination of nn.LogSoftmax() and nn.NLLLoss()
        ...
        for epoch in range(self.epochs):  
            ...
            print("\n starting time of this epoch = ", start_epoch)
            for i, data in enumerate(self.train_data_loader):
                ...
                #print("\n time taken = %f ms "%(time.time()-start_epoch)*1000)
                #start_epoch = time.time()
...
#                    #print("[epoch:%d, batch:%5d] loss: %.3f" % (epoch + 1, i + 1, running_loss / float(2000)))
        ...
        #print("\n time taken for one epoch = %f ms " %(time.time()-start_time)*1000)
        self.save_model(model)
