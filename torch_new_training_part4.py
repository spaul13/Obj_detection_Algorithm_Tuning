...
log_step = 600#990#450 #990 #450
comp_techs = 3
path_saved_model = "C:\\temp\\sibu_0330_ffmpeg_model"
...
model_det = Darknet(cfgfile)
model_det.load_weights(weightsfile)
...
def detect(det_file, reso, logfile):
    model_det.net_info["height"] = reso
    inp_dim = reso
    ...
    for batch in im_batches:
        ...
        with torch.no_grad():
            prediction = model_det(Variable(batch), CUDA) 
...
class Detection_net(nn.Module):
    ...
    class Net(nn.Module):
        def __init__(self):
            ...
            """
            self.do11 = nn.Dropout(p=0.1)
            self.fc11 = nn.Linear(50176, 1024)
            self.relu11 = nn.ReLU(inplace=True)
            self.fc12 = nn.Linear(1024, 256)
            self.relu12 =nn.ReLU(inplace=True)
            self.do12 = nn.Dropout(p=0.1)
            self.fc13 = nn.Linear(256, comp_techs)
            """
        def forward(self, x):
            x = self.conv_seqn(x)
            # flatten
            ...
            x = self.fc_seqn(x)
            #decomposed/split
            #x = self.do12(self.relu12(self.fc12(self.relu11(self.fc11(self.do11(x))))))
            #x = self.fc13(x)
            return x
    ...
    def training_code(self, model):        
        ...
        for epoch in range(self.epochs):  
            ...
            for i, data in enumerate(self.train_data_loader):
                ...
                #print("\n for regression model :", outputs, outputs.requires_grad)
                ...
                #loss.register_hook(lambda grad: #print(grad)) #this should always give 1 = dLoss/dloss
                #print("\n Before backward loss gradient : ",loss.grad,loss.data)
                #before backward() each layer has zero gradient
                loss.backward()
                """
                #logging the backpropagation
                #print("\n After backward loss gradient : ",loss.grad,loss.data)
                #print("\n loss backprop : ", loss.grad_fn, loss.grad_fn.next_functions[0][0].next_functions[0][0])
                x = loss.grad_fn
                #print(x)  # MSELoss
                x = x.next_functions
                #print(x[0][0], len(x))  # Linear
                x = x[0][0].next_functions
                #print(x[0][0], len(x))
                x = x[0][0].next_functions
                #print(x[0][0], len(x))
                x = x[0][0].next_functions
                ...
                while len(x[0][0].next_functions)!= 0 :
                    #print(x[0][0], len(x.next_functions))
                    x = x[0][0].next_functions
                #print("\n outputs.grad : ", outputs.grad)
                """
                optimizer.step()
                #after backward its nonzero gradient
                ##print("\n last fc layer grad : " ,model.fc13.weight.grad)
...
#                    #print("[epoch:%d, batch:%5d] loss: %.3f" % (epoch + 1, i + 1, running_loss / float(2000)))
    ...
    def compress(self, infile, file_index, reso_weight, jpeg_weight, encode_weight):
        ...
        encoding_qp = 5 + int(best_qp*encode_weight)
        jpeg_qp = 8 + int(best_jpeg*jpeg_weight)#higher the better
        """
        dim = (width, height)
        #print(dim)
        ...
        #JPEG compression
        jpeg_qp = int(best_jpeg*jpeg_weight)#higher the better
        cv2.imwrite("temp.jpg", resized, [int(cv2.IMWRITE_JPEG_QUALITY), jpeg_qp])
        #encoding
        encoding_qp = int(best_qp/encode_weight)
        cmd_str = "ffmpeg -i temp.jpg -c:v libx264 -qp " + str(encoding_qp) +" temp.mp4"
        os.system(cmd_str)
        """
        cmd_str = "ffmpeg -threads 4 -i " + infile +" -vf scale=" + str(width)+":" + str(height) + " -q:v "+ str(jpeg_qp) +" -c:v libx264 -qp " + str(encoding_qp) + " -loglevel quiet -y temp_1.mp4"
        os.system(cmd_str)
        print("\n file_index = %d, current resolution is = %d, jpeg qp = %d, encode qp = %d" % (file_index, width, jpeg_qp, encoding_qp))
        #revert back(need this?)
        """
        vidObj = cv2.VideoCapture("temp.mp4")
        success, image = vidObj.read() 
        #dim = (3840,2160)
        resized = cv2.resize(image, (det_reso[0],det_reso[1]), interpolation = cv2.INTER_AREA)
        cv2.imwrite("temp_1_org.png", resized, [int(cv2.IMWRITE_PNG_COMPRESSION), 0])
        """
        cmd_str = "ffmpeg -threads 4 -i temp_1.mp4 -vf scale=3840:2160 -loglevel quiet -y temp_2_org.png"
        os.system(cmd_str)
        acc_loss = self.find_acc_loss(infile, "temp_2_org.png")
        ...
        acc_loss += self.check_size("temp_1.mp4")
        return acc_loss
        
    #finds the accuracy loss between original image and preprocessed image
    ...
    def find_acc_loss(self, orgfile, testfile):
        ...
        #print("\n detecting the original file \n")
        #ind = detect(orgfile, 1024, "log_org")
        self.pic_number+=1
        #print("\n detecting the test file \n")
